...
- Add index to edit_log table timestamps to resolve timeouts on instances with lots of resource changes [#7853](https://github.com/archesproject/arches/issues/7853)
...
- Make node field on graph not nullable [#12103](https://github.com/archesproject/arches/issues/12103)
...
- Enforce single default language [#12027](https://github.com/archesproject/arches/pull/12027)
...
- Django-oauth-toolkit has been upgraded to 3.0.1.  
...
1. Upgrade to Arches 8.0.0:
    ...
    pip install --upgrade "arches>=8.0.0,<8.1.0
    ```
...
1. In settings.py:
    - Add the following key to `DATABASES["default"]` to [potentially improve indexing performance](https://github.com/archesproject/arches/issues/11382):
        ...
        ```
    - Change the `"captcha"` entry in `INSTALLED_APPS` to reflect its new name, and add `"pgtrigger"` and `"django_sql_migrate"`:
        ```
        INSTALLED_APPS = [
            ...
...
1. Update your project's `package.json`:
    ...
    - Update the dependency references to `stable/8.0.0`:
    ```    
    "dependencies": {
...
1. (optional) If performing reloads of your arches graphs and business data from a package generated in arches 7.x - you should update your package by exporting your graphs and business data again after the upgrade.
